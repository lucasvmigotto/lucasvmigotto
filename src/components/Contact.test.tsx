...
describe("Contact", () => {
  test("renders mailto button and email address", () => {
    ...
    expect(getByText(meta.whatsappUsername)).toBeTruthy();
  });
  ...
  test("whatsapp link target blank", () => {
    const { getByText } = renderWithI18n(<Contact meta={meta} />);
    const whatsappAnchor = getByText(meta.whatsappUsername).closest("a");
    expect(whatsappAnchor?.getAttribute("href")).toBe(meta.whatsappLink);
    expect(whatsappAnchor?.getAttribute("target")).toBe("_blank");
  });
});
